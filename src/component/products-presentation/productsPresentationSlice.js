...
export const getTypeOfProduct = createAsyncThunk(
   ...
   async (_, { getState }) => {
      ...
      try {
         ...
         if (state.productsPresentationData.typeProduct == typeProduct.ACCESSORIES) {
...
export const filterByAll = createAsyncThunk(
   ...
   async (_, { getState, dispatch }) => {
     const state = getState();
     try {
       const res = await api.get('products/filter', { params: state.productsPresentationData.filter });
       const data = await res.data;
       console.log(data);
       dispatch(productsPresentationSlices.actions.setStar({ key: '', star: 0 }));
       return data;
     } catch (error) {
       console.log(error);
       throw error;
...
export const productTypeSelector = state => state.productsPresentationData.listTypeOfProduct

export const categorySelector = state => state.productsPresentationData.typeProduct

export const filterObjectSelector = state => state.productsPresentationData.filter
